...
const ChatBot: React.FC = () => {
  ...
  /* Mensaje inicial al abrir o cambiar idioma */
  useEffect(() => {
    setMessages([{ role: 'bot', text: t.chat_initial }]);
  }, [language, t.chat_initial]);
        ...
        <div className="bg-white w-80 md:w-96 rounded-3xl shadow-2xl flex flex-col overflow-hidden border border-gray-100 animate-in slide-in-from-bottom-4 duration-300">
          ...
          <div className="bg-gray-900 text-white p-4 flex justify-between items-center">
            <div className="flex items-center space-x-3">
              <div className="w-8 h-8 bg-violet-400 rounded-full flex items-center justify-center text-xs font-bold shadow-lg">
                M
              </div>
          ...
          <div
            ref={scrollRef}
            className="h-96 overflow-y-auto p-4 space-y-4 bg-warm-white"
          >
            {messages.map((m, i) => (
              <div
                key={i}
          ...
          <div className="p-4 bg-white border-t border-gray-100 flex items-center space-x-2">
            <input
              ...
              placeholder="…"
              className="flex-1 bg-gray-50 rounded-xl p-3 text-sm focus:ring-1 focus:ring-violet-400 outline-none"
            />
            <button
              onClick={handleSend}
