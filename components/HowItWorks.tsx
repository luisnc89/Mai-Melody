...
const HowItWorks: React.FC<HowItWorksProps> = ({ language, onCtaClick }) => {
  ...
  return (
    <section id="funciona" className="py-24 px-4 bg-warm-white relative overflow-hidden">
      {/* Subtle background musical note */}
      <div className="absolute top-1/4 right-10 text-[12rem] text-violet-100/40 select-none font-serif -z-10">♬</div>
      <div className="absolute bottom-10 left-10 text-4xl butterfly-float text-violet-300/30">🦋</div>

      <div className="max-w-7xl mx-auto relative z-10">
        <div className="text-center mb-20 space-y-4">
          <h2 className="text-5xl lg:text-7xl font-serif text-gray-900">{t.how_title}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-8 lg:gap-12 relative">
          ...
          <div className="hidden md:block absolute top-14 left-[10%] right-[10%] h-px border-t border-dashed border-violet-200 -z-10"></div>
            ...
            <div key={idx} className="flex flex-col items-center group">
              <div className="relative mb-6">
                ...
                <div className="w-28 h-28 bg-white rounded-[2rem] shadow-[0_20px_40px_rgba(139,92,246,0.06)] flex items-center justify-center text-4xl transform transition-all duration-500 group-hover:scale-110 group-hover:shadow-violet-200/50 border border-violet-50">
                  <span className="group-hover:rotate-12 transition-transform duration-300">{step.icon}</span>
                </div>
