...
interface PricingProps {
  language: Language;
  onPackSelected: (pack: PackType) => void;
}

const Pricing: React.FC<PricingProps> = ({ language, onPackSelected }) => {
    ...
    <section className="py-20 px-4 bg-transparent relative">
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="text-center mb-16 space-y-4">
          <h2 className="text-4xl lg:text-5xl font-serif text-gray-900">{t.pricing_title}</h2>
          <p className="text-gray-600 max-w-2xl mx-auto font-medium">{t.pricing_desc}</p>
          ...
          {packs.map((pack) => (
            ...
            >
              ...
              <div className="space-y-6">
                <h3 className="text-2xl font-serif text-gray-800">{pack.name}</h3>
                <div className="flex items-baseline space-x-2">
                  <span className="text-5xl font-bold bg-gradient-to-br from-gray-900 to-violet-800 bg-clip-text text-transparent">
                    {pack.price}€
                  </span>
                  <span className="text-gray-500 text-sm italic">{t.iva_included}</span>
                </div>
              ...
              >
                <span className="relative z-10">{t.select_pack}</span>
              </button>
            </div>
          ))}
        </div>
